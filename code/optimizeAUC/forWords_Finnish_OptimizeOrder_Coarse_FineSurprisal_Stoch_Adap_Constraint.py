#assert False, "just so it doesn't go on forever"
...
parser.add_argument("--constraint", type=str, default=random.choice(["SVO", "same"])) #  "SOV", "VSO",

args=parser.parse_args()
assert args.constraint in ["SVO", "SOV", "VSO", "same"]
...
while True:
   ...
   itos_pure_deps_summed = [x for x in itos_pure_deps_ if x in ["nsubj", "obj", "HEAD"]]
   itos_pure_deps_summed = "".join([{"nsubj" : "S", "HEAD" : "V", "obj" : "O"}[x] for x in itos_pure_deps_summed])
   assert itos_pure_deps_summed in ["SVO", "SOV", "VSO", "OVS", "VOS", "OSV"]
   if args.constraint == "same" and itos_pure_deps_summed in [ "SOV", "VSO", "VOS", "OSV"]:
       break
   if itos_pure_deps_summed == args.constraint:
      break
#random.choice(["SVO", "SOV", "VSO"])
...
#  coordinate = "root"
 # while coordinate in ["HEAD", "root", "nsubj", "obj"]:
  ...
  for newValue in [choice([-1] + [2*x+1 for x in range(len(itos_pure_deps))])]:
     ...
     if args.constraint == "same":
        if (weights_["obj"] < weights_["HEAD"]) != (weights_["nsubj"] < weights_["HEAD"]):
            continue
     else:
        if (weights_["obj"] < weights_["nsubj"]) != (args.constraint.index("O") < args.constraint.index("S")):
           continue
        if (weights_["obj"] < weights_["HEAD"]) != (args.constraint.index("O") < args.constraint.index("V")):
           continue
        if (weights_["nsubj"] < weights_["HEAD"]) != (args.constraint.index("S") < args.constraint.index("V")):
           continue
     # Calculate AUC for this updated assignment
     resultingAUC, _ = calculateTradeoffForWeights(weights_)

     # Update variables if AUC is smaller than minimum AUC found so far
     if resultingAUC < bestAUCSoFar:
...
#       os.makedirs(TARGET_DIR)
     TARGET_DIR = "/u/scr/mhahn/deps/hillclimbing-auc_constrained/"
     with open(TARGET_DIR+"/optimized_"+args.language+"_"+__file__+"_"+str(myID)+".tsv", "w") as outFile:
        print >> outFile, (iteration, lastUpdated, bestAUCSoFar, str(args), surprisals)
