...
if True:
     with open("output/"+__file__+".tsv", "a") as outFile:
        ...
        if order.index("S") > order.index("V"):
            order = order[::-1]
        correl = [x for x in ["lifted_case", "lifted_cop", "lifted_mark", "nmod", "obl", "xcomp", "acl", "aux"] if x in weights]
        def d(x):
        ...
        print >> outFile, "\t".join([str(w) for w in (args.language, args.group, args.model, order, np, abs(np.index("D") - np.index("_")), abs(np.index("N") - np.index("_")), abs(np.index("A") - np.index("_"))), len(satisfiedCorrelations)])
