...
def orderSentence(sentence, weights):
   ...
   for line in sentence:
     line["children_DH"] = []
     line["children_HD"] = []
#     assert line["dep"] != "case" 
   for line in sentence:
#      assert line["dep"] != "case" , line
 #     assert line["coarse_dep"] != "case" , line
      line["fine_dep"] = line["dep"]
      if line["fine_dep"] == "root":
          root = line["index"]
          continue
...
def calculateTradeoffForWeights(weights, getSurprisal=False):
...
#    quit()
    ...
    if getSurprisal:
       ...
       auc, devSurprisalTable = 0, None
    print("VALUES", auc, totalDependencyLength/totalWordCount)
    overallObjective = auc + totalDependencyLength/totalWordCount
...
#  assert bestObjectiveSoFar < 1e99
 ...
 # print(weights)
  ...
  if (iteration + 1) % 500 == 0: # and iteration-lastUpdated <= 100:
     _, surprisals, dependencyLength = calculateTradeoffForWeights(weights_, getSurprisal=True)

#     if os.path.exists(TARGET_DIR):
#       pass
#     else:
#       os.makedirs(TARGET_DIR)
     TARGET_DIR = "/u/scr/mhahn/deps/FUNCHEAD_hillclimbing-auc-onlydlm/"
