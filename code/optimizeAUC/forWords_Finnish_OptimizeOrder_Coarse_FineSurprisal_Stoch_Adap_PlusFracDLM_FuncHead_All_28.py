...
if len(sys.argv) > 1:
   ...
   N =1000
import glob
BASE = "FUNCHEAD_hillclimbing-auc-fracdlm"
script = "forWords_Finnish_OptimizeOrder_Coarse_FineSurprisal_Stoch_Adap_PlusFracDLM_FuncHead.py"
...
for _ in range(N):
  ...
  #      language = "German-GSD_2.8"
     ...
     if len(glob.glob(f"/u/scr/mhahn/deps/{BASE}/optimized_{language}_{script}*tsv")) + len(glob.glob(f"/u/scr/mhahn/deps/{BASE}/optimized_{language.replace('2.8', '2.6')}_{script}*tsv")) < 30:
         found = True
         break
   if not found:
     break
