...
languages = set()
with open("output/grammars.tsv", "w") as outFile:
 print("\t".join(["idForProcess", "language", "dependency", "weight"]), file=outFile)
 with open("output/arguments.tsv", "w") as outFileArgs:
  ...
  for f in sorted(files):
   ...
   for dependency, weight in data.items():
      print("\t".join([args["idForProcess"], args["language"].strip("'"), dependency, weight]), file=outFile)
